...
is the number of times you have to replace n with the sum of its digits until n becomes a single digit integer.

The multiplicative persistence of an integer, n,
is the number of times you have to replace n with the product of its digits until n becomes a single digit integer.
...
def additive_persistence(number):
    ...
    while number // 10 != 0:
        count += 1
        number = functools.reduce(lambda a, b: a + b, list(map(int, str(number))))
